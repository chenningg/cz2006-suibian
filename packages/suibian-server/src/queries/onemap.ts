...
export async function getRouteDetailsSocketless(
  ...
  if (startLocationData && endLocationData) {
    ...
    const { lat, lon } = startLocationData;
    const start_lat = lat.toString();
    const start_lon = lon.toString();
...
async function test() {
  const endpostal = "199583";
  const test2 = JSON.stringify({ lat: 1.3521, lon: 103.8198 });
  console.log(await getRouteDetailsSocketless(test2, endpostal, token));
}
